...
(function() {
	...
	var n = window, i = {
		innerHTML : ""
	}, z = "document", B = "documentElement", H = "getElementsByTagName", E, u, h, f, D;
					...
					if (e) {
						for ( var L in e) {
							if (this.dt[L] === undefined) {
								...
								if (L == "M") {
									var M = e.M > 0 ? 1 : 0;
									var K = new Date(this.dt.y, this.dt.M, 0)
											.getDate();
	...
	function x(e) {
		e = e || E;
		var J = e[z], I = (e.innerWidth) ? e.innerWidth
				: (J[B] && J[B].clientWidth) ? J[B].clientWidth
						: J.body.offsetWidth, d = (e.innerHeight) ? e.innerHeight
				: (J[B] && J[B].clientHeight) ? J[B].clientHeight
						: J.body.offsetHeight;
		return {
			width : I,
			height : d
	...
	function F(e) {
		...
		J = (d && d.scrollTop != null && (d.scrollTop > I.scrollTop || d.scrollLeft > I.scrollLeft)) ? d
				: I;
		return {
			top : J.scrollTop,
			left : J.scrollLeft
	...
	function s(d) {
		try {
			...
			if ($dp.cal && !$dp.eCont && $dp.dd && w != $dp.el
			...
			if (J.eCont) {
				...
				try {
					if (!J.el
							|| J.el.My97Mark === true
							|| J.el.disabled
							|| ($dp.dd && r($dp.dd) != "none" && $dp.dd.style.left != "-970px")) {
						if (J.el.My97Mark) {
							J.el.My97Mark = false
						}
						return
	...
	function c(e, d) {
		return e.currentStyle ? e.currentStyle[d] : document.defaultView
				.getComputedStyle(e, false)[d]
	}
	function r(e, d) {
		if (e) {
			if (d != null) {
	...
	function a(e, d) {
		...
		if (e.lang == "auto") {
			e.lang = h ? navigator.browserLanguage.toLowerCase()
		...
		if (!$dp.dd
				|| e.eCont
				|| ($dp.dd && (e.getRealLang().name != $dp.dd.lang || e.skin != $dp.dd.skin))) {
			if (e.eCont) {
				w(e.eCont, e)
			} else {
		...
		function w(V, P) {
			...
			try {
				...
				N.frameBorder = 0;
				N.scrolling = "no";
				N.src = "javascript:(function(){var d=document;d.open();d.domain='"
						+ O + "';})()";
				V.appendChild(N);
				setTimeout(function() {
					T = V.lastChild.contentWindow[z];
			...
			function R() {
				...
				var X = [
						...
						"<\/script><script src=", u, "lang/", Y.name,
						".js charset=", Y.charset, "><\/script>" ];
				if (S) {
					X[1] = 'document.domain="' + O + '";'
				}
				for ( var W = 0; W < U.length; W++) {
					if (U[W].name == P.skin) {
						X.push('<link rel="stylesheet" type="text/css" href="'
								+ u + "skin/" + U[W].name
								+ '/datepicker.css" charset="' + U[W].charset
								+ '"/>')
					}
				}
		...
		function I(O) {
			...
			M += Q.left + Math.min(P.leftM + T.left, U.width - S - 5)
					- (h ? 2 : 0);
			O.dd.style.top = V + "px";
